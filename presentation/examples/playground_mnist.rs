...
use iced::widget::{button, Canvas, column, container, pick_list, progress_bar, row, slider, text, text_input};
...
fn main() -> iced::Result {
    ...
    .window_size(iced::Size::new(1500.0, 900.0))
...
impl PlaygroundApp {
        ...
        match message {
            ...
            Message::LoadPreset(preset) => {
                self.layers = match preset {
                    PresetType::Shallow => vec![LayerConfig::new(128, self.global_activation)],
                    PresetType::Deep => vec![
    ...
    fn view(&self) -> Element<'_, Message> {
        let mut content_items = vec![
            text("Neural Network Playground - MNIST Edition")
                .size(32)
                .into(),
            text("Configure your network architecture, train on MNIST, and test with hand-drawn digits")
                .size(16)
                .into(),
        ];
        ...
        let content = column(content_items).spacing(20).padding(20);
    ...
    fn network_config_panel(&self) -> Element<'_, Message> {
        ...
        column![
            text("Network Architecture").size(20),
            text(architecture).size(14),
            text(format!("Total parameters: {}", total_params)).size(12),
            text("Presets").size(16),
            row![
                button(text("Shallow")).on_press(Message::LoadPreset(PresetType::Shallow)),
                button(text("Deep")).on_press(Message::LoadPreset(PresetType::Deep)),
                button(text("Pyramid")).on_press(Message::LoadPreset(PresetType::Pyramid)),
                button(text("Wide")).on_press(Message::LoadPreset(PresetType::Wide)),
            ]
            .spacing(8),
            text("Layer Management").size(16),
            row![
                ...
                button(text("Reset")).on_press(Message::ResetNetwork),
            ]
            .spacing(8),
            text("Layers").size(16),
            column(
                self.layers
                    ...
                    .map(|(idx, layer)| {
                        row![
                            ...
                            text(format!("{}", layer.neurons)).size(12),
                        ]
                        .spacing(8)
                        .into()
                    })
    ...
    fn training_panel(&self) -> Element<'_, Message> {
        let status_text = match self.training_state {
            TrainingState::Idle => {
                if let Some(result) = &self.training_result {
                    ...
                    "Configure and train your network".to_string()
                }
            }
            ...
            TrainingState::Paused => "Training paused".to_string(),
        };

        column![
            text("Training Configuration").size(20),
            text(status_text).size(14),
            row![
                text("Epochs:").width(Length::Fixed(100.0)),
            ...
            .spacing(10),
            row![
                text("Learning Rate:").width(Length::Fixed(100.0)),
            ...
            .spacing(10),
            row![
                text("Batch Size:").width(Length::Fixed(100.0)),
    ...
    fn canvas_panel(&self) -> Element<'_, Message> {
        ...
        let bars: Vec<Element<Message>> = (0..10)
            .map(|digit| {
                ...
                row![
                    ...
                    text(format!(" {}", percentage_text)).width(Length::Fixed(60.0)),
                ]
                .spacing(5)
        ...
        let prediction_header = match &self.prediction {
            None => column![
                text("Predicted: —").size(24),
                text("Confidence: —").size(16),
            ],
            Some(pred) => column![
                text(format!("Predicted: {}", pred.digit)).size(24),
                text(format!("Confidence: {:.1}%", pred.confidence * 100.0)).size(16),
            ],
        };

        // Canvas and controls on the left
        let canvas_section = column![
            text("Draw a Digit").size(20),
            canvas_widget,
            row![
                button(text("Clear")).on_press(Message::ClearCanvas),
        ...
        let histogram_section = column![
            text("Prediction").size(20),
            prediction_header,
            text("").size(10), // Spacer
            text("Probability Distribution:").size(16),
            column(bars).spacing(8),
        ]
        .spacing(10)
        .width(Length::Fixed(400.0));
